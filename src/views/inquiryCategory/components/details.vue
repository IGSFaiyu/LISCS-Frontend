<template>
  <div class="app-container">
    <div class="page-title">
      <span class="title">Inquiry Category - {{ title }}</span>
      <div>
...
async function getDetails() {
  try {
    let { code, data, message } = await getInCategoryDetails(route.query.categoryId);
    if (code === 200) {
      formData.value = { ...data };
    } else {
...
async function handleInfo() {
  if (title.value === "Create") {
    try {
      ...
      let { code, data, message } = await addInCategory(params);
      if (code === 200) {
    ...
    try {
      ...
      let { code, data, message } = await updateInCategory(params);
      if (code === 200) {
        ElMessage.success(message);
        router.go(-1);
      } else {
