<template>
  <div class="app-container">
    <div class="page-title">
      <span class="title">Inquiry Category</span>
...
import { getInCategoryList, deleteInCategory } from "@/api/index";
...
async function getData() {
  try {
    ...
    let { code, data, message } = await getInCategoryList(params);
    if (code === 200) {
...
function handleAdd() {
  // 跳转至详情页
  router.push("/addInCategory");
}

async function handleDelete(params) {
  try {
    let { code, data, message } = await deleteInCategory(params.categoryId);
    if (code === 200) {
