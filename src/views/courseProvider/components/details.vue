<template>
  <div class="app-container">
    <div class="page-title">
      <span class="title">Course Provider - {{ title }}</span>
      <div>
    ...
    <tiny-form ref="ruleFormRef" :model="formData" label-position="top">
      <tiny-row :flex="true">
        ...
        <tiny-col :span="5">
          <tiny-form-item label="Name (English)" prop="nameEng">
...
async function getDetails() {
  try {
    let { code, data, message } = await getCourseProDetails(
...
async function handleInfo() {
  if (title.value === "Create") {
    try {
      ...
      let { code, data, message } = await addCoursePro(params);
      if (code === 200) {
    ...
    try {
      ...
      let { code, data, message } = await updateCoursePro(params);
      if (code === 200) {
        ElMessage.success(message);
        router.go(-1);
      } else {
