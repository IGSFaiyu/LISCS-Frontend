...
import { getCurrencyList, deleteCurrency } from "@/api/index";
...
const allColumns = ref("currencyCode, descChi, descEng, rate, modifiedOn, modifiedBySnap")

const sortColumn = ref("currencyCode");
...
async function getData() {
  try {
    ...
    let { code, data, message } = await getCurrencyList(params);
    if (code === 200) {
...
async function handleDelete(params) {
  try {
    let { code, data, message } = await deleteCurrency(params.currencyCode);
    if (code === 200) {
