<template>
  <div class="app-container">
    <div class="page-title">
      <span class="title">Job Nature</span>
...
import { getJobNatureList, deleteJobNature } from "@/api/index";
...
const allColumns = ref("category, descChi, descEng, triggerField, modifiedOn, modifiedBySnap")

const sortColumn = ref("natureId");
...
async function getData() {
  try {
    let params = {
      ...
      triggerField: form.value.triggerField,
    };
...
function handleAdd() {
  // 跳转至详情页
  router.push("/addJobNature");
}

async function handleDelete(params) {
  try {
    let { code, data, message } = await deleteJobNature(params.natureId);
    if (code === 200) {
