<template>
  <div class="app-container">
    <div class="page-title">
      <span class="title">Problematic Area</span>
...
import { getProAreaList, deleteProArea } from "@/api/index";
...
async function getData() {
  try {
    let params = {
      ...
      mappedField: form.value.mappedField,
    };
    let { code, data, message } = await getProAreaList(params);
    if (code === 200) {
...
function handleAdd() {
  // 跳转至详情页
  router.push("/addProblematic");
}

async function handleDelete(params) {
  try {
    let { code, data, message } = await deleteProArea(params.areaId);
    if (code === 200) {
